...
def index(request):
    ...
    schema = data.get("http://127.0.0.1:2469/farm")
    ...
    with open('static/json/well.json', 'w') as outfile:
        json.dump(schema, outfile)
    schema = data.get("http://127.0.0.1:2469/member")
    with open('static/json/member.json', 'w') as outfile:
        json.dump(schema, outfile)
    schema = data.get("http://127.0.0.1:2469/season")
    with open('static/json/season.json', 'w') as outfile:
        json.dump(schema, outfile)
    schema = data.get("http://127.0.0.1:2469/crop")
